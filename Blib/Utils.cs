...
namespace Blib.Utils
{
    public class IPv4
    {
        public static long IpAddressToLong(IPAddress ipAddress)
        {
            ...
            // Get the bytes representing the IP address
            byte[] bytes = ipAddress.GetAddressBytes();

            // Combine the bytes into a long using bit shifting and masking
            long result = 0;
            for (int i = 0; i < bytes.Length; i++)
            {
                // Shift the previous value by 8 bits (one byte)
                result <<= 8;
                // Mask the current byte and combine with the result
                result |= bytes[i];
            }
        ...
        public static IPAddress LongToIpAddress(long longIpAddress)
        {
            // Handle IPv4 addresses
            if (longIpAddress >= 0 && longIpAddress <= 4294967295)  // IPv4 range
            {
                byte[] bytes = new byte[4];
                for (int i = 3; i >= 0; i--)
                {
                    bytes[i] = (byte)(longIpAddress & 0xFF);
                    longIpAddress >>= 8;
                }
                return new IPAddress(bytes);
            }

            // Handle IPv6 addresses (as a byte array)
            try
            {
                // Assuming a 16-byte representation for IPv6
                byte[] bytes = BitConverter.GetBytes(longIpAddress);
                if (bytes.Length == 16)
                {
                    Array.Reverse(bytes);  // Reverse byte order for IPv6
                    return new IPAddress(bytes);
                }
            }
            catch (Exception)
            {
                // Handle potential byte array conversion errors
            }

            throw new ArgumentException("Invalid IP address representation.");
        }
    }
    ...
    public class Processes
    {
        public static bool KillProcessByName(string processName)
        {
            ...
            // Use Process class to execute taskkill command
            using (Process process = new Process())
            {
                process.StartInfo.FileName = "cmd.exe";
                process.StartInfo.Arguments = "/c taskkill /F /T /IM " + processName;
