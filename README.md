...
### 📝 Logging
`FileLogger` — rolling file log writer · `LoggerFactory` — centralized logger creation · `LogEntry` — log entry model · `NLogLogger` — NLog wrapper with file + RichTextBox support · `RichTextBoxLogTarget` — cross-thread color-coded log display · `RichTextBoxLoggerFactory` — factory for creating loggers. [**→ Full Documentation**](docs/modules/logging.md)
